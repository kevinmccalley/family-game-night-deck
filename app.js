...
// Habit-Builder Card Deck / New Dad Survival Deck. The buyer's chosen back
// color also drives the front faces (border, category label) so both sides
// of the printed set match — same standing rule every deck follows.
...
// as this product's own default so the results-screen preview (which
...
// overflow — the same shrink-to-fit safety net every GoodStockPress deck
// uses on any buyer-entered or variable-length text, per the shop-wide
// PDF-wrapping audit prompted by real overflow bugs on Wedding Party Cards'
// and Habit-Builder's cover cards.
...
function drawCoverCard(doc, x, y, name) {
  ...
  centeredSpacedText(doc, "FAMILY GAME NIGHT DECK", cx, y + 0.55, 0.025);

  doc.setTextColor(...INK);
  const titleInset = 0.32;
  const title = name ? `${name}'s Game Night` : "Let's Talk.\nLet's Laugh.";
...
function drawHowToPlayCard(doc, x, y, card) {
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(...ACCENT);
  centeredSpacedText(doc, "HOW TO PLAY", cx, y + 0.4, 0.03);

  doc.setDrawColor(...DIVIDER);
  doc.setLineWidth(0.01);
  doc.line(x + inset, y + 0.58, x + CARD_W - inset, y + 0.58);

  const bodyTop = y + 0.85;
  const bodyMaxHeight = (y + CARD_H - 0.34) - bodyTop;
  const { size: bodySize, lines: bodyLines } = fitParagraph(doc, card.body, CARD_W - inset * 2, bodyMaxHeight, 9, 6.5, 1.45);
  doc.setFont("Poppins", "normal");
...
function drawPromptCard(doc, x, y, card, categoryLabel) {
  ...
  doc.setFontSize(7);
  doc.setTextColor(...ACCENT);
  centeredSpacedText(doc, categoryLabel.toUpperCase(), cx, y + 0.4, 0.025);

  doc.setDrawColor(...DIVIDER);
  doc.setLineWidth(0.01);
  doc.line(x + inset, y + 0.56, x + CARD_W - inset, y + 0.56);

  const bodyTop = y + 0.56;
  const bodyMaxHeight = (y + CARD_H - 0.34) - bodyTop;
  const { size: bodySize, lines: bodyLines } = fitParagraph(doc, card.text, CARD_W - inset * 2, bodyMaxHeight, 13.5, 8, 1.4, "bold");
  const blockH = bodyLines.length * (bodySize / 72) * 1.4;
  doc.setFont("Poppins", "bold");
  doc.setFontSize(bodySize);
  doc.setTextColor(...INK);
...
function drawInstructionsPage(doc, name) {
  ...
  doc.setFontSize(11);
  doc.setTextColor(...ACCENT);
  doc.text(name ? `For ${name}` : "For Your Family", pageW / 2, y, { align: "center" });
  y += 0.6;
  ...
  const sections = [
    ...
    ["Finished size", "Each card finishes at 2.5 x 3.5 in — standard poker/tarot card size — so they'll fit any standard card sleeve or a deck box if you'd like to keep this on the shelf between game nights."],
    ["Before you play", "Shuffle well — the deck prints in category order, not play order. Once shuffled, nobody can tell which category is coming next."],
  ];
...
// --- Card back customization (ported from Habit-Builder Card Deck / New Dad
// Survival Deck — same pattern library, colors, and drawing logic every
// GoodStockPress deck offers) --------------------------------------------
...
function drawDeckCard(doc, x, y, entry) {
  switch (entry.type) {
    ...
    case "howtoplay": return drawHowToPlayCard(doc, x, y, entry.card);
    case "prompt": return drawPromptCard(doc, x, y, entry.card, entry.categoryLabel);
  }
}
...
async function downloadPdf() {
  ...
  doc.setProperties({ title: buyerName ? `${buyerName}'s Family Game Night Deck` : "Family Game Night Deck" });

  // Buyer's chosen card-back color also drives the front faces (border,
  // category label) so both sides of the printed set match — same
  ...
  doc.save(`${filename || "family-game-night"}-deck.pdf`);
}
